/**
 * Creepy Proximity Ghost (Simplified) → Bluetooth Speaker (A2DP Source)
 ...
 *
 * DESIGN GOAL (Very Simple):
 *  - No complicated AM/FM stacks. Just a single, smooth "ghost" tone.
 ...
 *
 * Keep this simple so it’s easy to tweak & debug.
 */
...
// REPLACE with your speaker’s MAC address bytes:
...
// Sonar behavior (you can keep your own pollSonar; this matches the earlier pattern)
...
// ---------------- Very small sine LUT + linear interpolation ----------------
...
// One carrier + one very slow "breath" LFO for gentle amplitude shaping
static Osc oscCar;
...
// Simple, non-invasive sonar polling (leave your own debug prints if you have them)
...
// ---------------- Smooth ghost parameters (very simple) ----------------
// We only smooth distance, then map to pitch + loudness. No FM/AM complexity.
...
// Tuning
constexpr float MASTER_GAIN    = 0.60f;   // overall loudness (cap so SBC doesn’t pump)
...
void setup() {
  ...
  pinMode(PIN_ECHO, INPUT);  // if you prefer: INPUT_PULLDOWN (divider still keeps safe)
...
// ---------------- Arduino Loop ----------------
void loop() {
  // If your core exposes a connected() method and you want to skip audio when not connected,
  // you can gate on it here. Keeping it simple: we fill and write regardless; sink can catch up.

  // 1) Update sonar
  pollSonar();

  // 2) Smooth distance & derive proximity
  //    Very gentle smoothing to avoid steps; tweak alphas to taste.
  ...
  // 3) Map to pitch + loudness (SIMPLE)
  //    - SILENCE when prox_sm == 0 (i.e., distance >= FAR_CM)
  ...
  // 4) Breath envelope (gentle only)
  //    Breath rate goes slightly faster as you get close, but stays slow.
  ...
  // 6) Stream to A2DP (same pattern as the working example)
